...
namespace Rivet {
  ...
  public:
    ...
    bool exists_csv (const std::string& name){
    ...
    /// @{
    float EPSTENSOR(FourMomentum p1, FourMomentum p2, FourMomentum p3, FourMomentum p4)
    {
      return -(p1.px()*p2.py()*p3.pz()*p4.E() -
	       p1.px()*p2.py()*p3.E() *p4.pz()-
	       p1.px()*p2.pz()*p3.py()*p4.E() +
	       p1.px()*p2.pz()*p3.E() *p4.py()+
	       p1.px()*p2.E() *p3.py()*p4.pz()-
	       p1.px()*p2.E() *p3.pz()*p4.py()-
	       p1.py()*p2.px()*p3.pz()*p4.E() +
	       p1.py()*p2.px()*p3.E() *p4.pz()+
	       p1.py()*p2.pz()*p3.px()*p4.E() -
	       p1.py()*p2.pz()*p3.E() *p4.px()-
	       p1.py()*p2.E() *p3.px()*p4.pz()+
	       p1.py()*p2.E() *p3.pz()*p4.px()+
	       p1.pz()*p2.px()*p3.py()*p4.E() -
	       p1.pz()*p2.px()*p3.E() *p4.py()-
	       p1.pz()*p2.py()*p3.px()*p4.E() +
	       p1.pz()*p2.py()*p3.E() *p4.px()+
	       p1.pz()*p2.E() *p3.px()*p4.py()-
	       p1.pz()*p2.E() *p3.py()*p4.px()-
	       p1.E() *p2.px()*p3.py()*p4.pz()+
	       p1.E() *p2.px()*p3.pz()*p4.py()+
	       p1.E() *p2.py()*p3.px()*p4.pz()-
	       p1.E() *p2.py()*p3.pz()*p4.px()-
	       p1.E() *p2.pz()*p3.px()*p4.py()+
	       p1.E() *p2.pz()*p3.py()*p4.px());
    }
    
    float phisign(FourMomentum jsa, FourMomentum jsb){
      FourMomentum p1( 1, 0, 0, 1);
      FourMomentum p2( 1, 0, 0,-1);
      float epst=EPSTENSOR(p1,jsa,p2,jsb);
      if (epst < 0.0) return -1;
      else return 1;
    }
    ...
    HiggsClassification classifyEvent(const Event& event, const STXS::HiggsProdMode prodMode ) {
      ...
      for ( const Particle &p : FS ) {
        ...
        sum += p.momentum();
        // skip higgs if undecayed 
        if ( p.pdgId() == PID::HIGGS ) { continue; }
	...
	//std::stringstream csv_tag;
	//csv_tag << "output/stxs_hjet_" << rand() % 100 << ".csv" << std::endl;
	//::cout << " [YACINE] : " <<  handler().runName() << " -- "<< name() << " -- "<< currentDateTime() << std::endl;
  //std::srand(std::time(0));
  csv_file_name = "HJET_STXS.csv";
  // if(exists_csv(csv_file_name)){
  //      csv_file_name = "./output/HJET_STXS_"+std::to_string(rand() % 10000)+".csv";
  //     usleep(0.01);
  //  }
  csv_file.open ( csv_file_name );
    ...
    csv_file << "," << "scale_" + std::to_string(iw);
	}
	csv_file << std::endl;
	// forcing the roduction mode to be VBF
        m_HiggsProdMode = STXS::VBF;
      }
    ...
    void analyze(const Event& event) {
      ...
      // define regions for caculating uncertanties
      /*
        Bin 0   :  n_jet >= 0
        Bin 1   :  n_jet >= 1
        Bin 2   :  n_jet >= 2
        // ----
        Bin 3   :  n_jet >= 2, m_jj > 60
        Bin 4   :  n_jet >= 2, m_jj > 120
        Bin 5   :  n_jet >= 2, m_jj > 350
        Bin 6   :  n_jet >= 2, m_jj > 700
        Bin 7   :  n_jet >= 2, m_jj > 1000
        Bin 8   :  n_jet >= 2, m_jj > 1500
        // ----
        Bin 9   :  n_jet >= 2, m_jj < 60
        Bin 10  :  n_jet >= 2, m_jj < 120
        Bin 11  :  n_jet >= 2, m_jj < 350
        Bin 12  :  n_jet >= 2, m_jj < 700
        Bin 13  :  n_jet >= 2, m_jj < 1000
        Bin 14  :  n_jet >= 2, m_jj < 1500
        // ----
        Bin 15  :  n_jet >= 2, m_jj > 350,  Higgs_pt > 200
        Bin 16  :  n_jet >= 2, m_jj > 350,  Higgs_pt < 200
        Bin 17  :  n_jet >= 2, m_jj > 350,  Higgs_pt > 200, PTHjj > 25
        Bin 18  :  n_jet >= 2, m_jj > 350,  Higgs_pt < 200, PTHjj > 25
        // ----
        Bin 19  :  n_jet >= 2, PTHjj > 25
        Bin 20  :  n_jet >= 2, Higgs_pt > 200
        Bin 21  :  n_jet >= 2, Higgs_pt < 200
      */
      if (cat.jets30.size() >= 0) hist_uncert_cats->fill(0, weight);
      if (cat.jets30.size() >= 1) hist_uncert_cats->fill(1, weight);
      if (cat.jets30.size() >= 2) hist_uncert_cats->fill(2, weight);
      ...
      if (cat.jets30.size()>=2) {
        ...
        if ((j1+j2).mass()  > 60  ) hist_uncert_cats->fill(3 , weight);
        if ((j1+j2).mass()  > 120 ) hist_uncert_cats->fill(4 , weight);
        if ((j1+j2).mass()  > 350 ) hist_uncert_cats->fill(5 , weight);
        if ((j1+j2).mass()  > 700 ) hist_uncert_cats->fill(6 , weight);
        if ((j1+j2).mass()  > 1000) hist_uncert_cats->fill(7 , weight);
        if ((j1+j2).mass()  > 1500) hist_uncert_cats->fill(8 , weight);

        if ((j1+j2).mass() <= 60  ) hist_uncert_cats->fill(9 , weight);
        if ((j1+j2).mass() <= 120 ) hist_uncert_cats->fill(10, weight);
        if ((j1+j2).mass() <= 350 ) hist_uncert_cats->fill(11, weight);
        if ((j1+j2).mass() <= 700 ) hist_uncert_cats->fill(12, weight);
        if ((j1+j2).mass() <= 1000) hist_uncert_cats->fill(13, weight);
        if ((j1+j2).mass() <= 1500) hist_uncert_cats->fill(14, weight);

        if ((j1+j2).mass()  > 350 && cat.higgs.pT() >  200) hist_uncert_cats->fill(15 , weight);
        if ((j1+j2).mass()  > 350 && cat.higgs.pT() <= 200) hist_uncert_cats->fill(16 , weight);
        if ((j1+j2+cat.higgs.momentum()).pt() <= 25 ) {
          if ((j1+j2).mass()  > 350 && cat.higgs.pT() >  200) hist_uncert_cats->fill(17 , weight);
          if ((j1+j2).mass()  > 350 && cat.higgs.pT() <= 200) hist_uncert_cats->fill(18 , weight);
        }
        if ((j1+j2+cat.higgs.momentum()).pt()  > 25 ) hist_uncert_cats->fill(19, weight);
      }
      if (cat.higgs.pT()  > 200) hist_uncert_cats->fill(20, weight);
      if (cat.higgs.pT() <= 200) hist_uncert_cats->fill(21, weight);
      
      double mjj    = cat.jets30.size() < 2 ? -99.0 : (cat.jets30[0].momentum() + cat.jets30[1].momentum()).mass();
      double yjj    = cat.jets30.size() < 2 ? -99.0 : deltaRap(cat.jets30[0],cat.jets30[1]);
      double etajj  = cat.jets30.size() < 2 ? -99.0 : deltaEta(cat.jets30[0],cat.jets30[1]);
      double phijj  = cat.jets30.size() < 2 ? -99.0 : deltaPhi(cat.jets30[0],cat.jets30[1]);
      double signedPhijj = cat.jets30.size() < 2 ? -99.0 : phijj * phisign(cat.jets30[0].momentum(), cat.jets30[1].momentum());
      double ptHjj  = cat.jets30.size() < 2 ? -99.0 : (cat.higgs.momentum() + cat.jets30[0].momentum() + cat.jets30[1].momentum()).pt();
      double ptj1   = cat.jets30.size() < 1 ? -99.0 : cat.jets30[0].pt();
      double ptj2   = cat.jets30.size() < 2 ? -99.0 : cat.jets30[1].pt();
      double ptj3   = cat.jets30.size() < 3 ? -99.0 : cat.jets30[2].pt();
      double etaj1  = cat.jets30.size() < 1 ? -99.0 : cat.jets30[0].eta();
      double etaj2  = cat.jets30.size() < 2 ? -99.0 : cat.jets30[1].eta();
      double etaj3  = cat.jets30.size() < 3 ? -99.0 : cat.jets30[2].eta();
      
      double phij1  = cat.jets30.size() < 1 ? -99.0 : cat.jets30[0].phi();
      double phij2  = cat.jets30.size() < 2 ? -99.0 : cat.jets30[1].phi();
      double phij3  = cat.jets30.size() < 3 ? -99.0 : cat.jets30[2].phi();


      csv_file << cat.jets25.size()    << ","
	       << cat.jets30.size()    << ","
	       << mjj                  << ","
	       << yjj                  << ","
	       << etajj                << ","
         ...
         << phij3                << ","
         << cat.stage0_cat       << ","
	       << cat.stage1_1_fine_cat_pTjet25GeV << ","
	       << cat.stage1_1_fine_cat_pTjet30GeV << ","
	       << weight;
      for (unsigned int i=0; i < 10; i++){
        csv_file << "," << weightevent->weights()[i];
      }
      csv_file << std::endl;
    }
    ...
    void finalize() {
      ...
      //                  hist_pT_Higgs,hist_y_Higgs,hist_pT_V,hist_pT_jet1,hist_deltay_jj,hist_dijet_mass,hist_pT_Hjj, hist_uncert_cats})
      //iscale(hist, sf);
    ...
    void initializeHistos(){
      ...
      hist_uncert_cats            = bookHisto1D("STXS_Uncertainty_cat"  ,31, 0, 31);
      ...
      hist_dijet_mass  = bookHisto1D("m_jj"     ,100,  0,4000);
  ...
  private:
    ...
    Histo1DPtr hist_stage0, hist_uncert_cats;
